...
> - **네이버 웹툰**의 도전만화·베스트도전은 상세 레이아웃이 정식과 달라(장르가 태그 목록에 섞이지 않고 별도 필드) **전용 파서로 분기**합니다. 페이지가 CSR이라 DOM 대신 페이지가 쓰는 내부 API(`/api/article/list/info`)를 브라우저 세션으로 호출해 읽습니다. 단 도전만화는 작가명이 `smil****`처럼 **마스킹된 아이디**로 내려오니 적재 후 확인이 필요합니다(로그에 경고).
...
> - `## 단행본` 섹션은 항상 빈 채로 생성됩니다(`works_type`엔 없는 값). 연재처 없이 단행본으로만 나온 작품을 손으로 넣는 자리입니다.
