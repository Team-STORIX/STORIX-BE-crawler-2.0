"""
fill_missing_genre.py — genre 가 비어있는 works 를 찾아 titles.txt 로 출력

genre 가 NULL 이거나 빈 문자열인 works 를 뽑아, works_type(웹툰 | 웹소설)별
섹션 헤더 형식으로 titles.txt 에 기록한다. 이 파일을 search_titles 모드에 넣어
재크롤 → batch import 하면 빈 장르가 채워진다.

(2026-07-30 validator 에 genre 필수화가 추가되어, 장르 빈 신규 레코드는 검수큐로
 간다. 이 스크립트는 '이미 DB 에 들어간' 장르 빈 기존 works 를 재크롤로 보강한다.)

기본은 읽기 전용(미리보기)이며, 실제 기록은 --write 를 줘야 한다.

    python scripts/fill_missing_genre.py                 # 대상만 출력 (파일 미변경)
    python scripts/fill_missing_genre.py --write         # titles.txt 에 기록
    python scripts/fill_missing_genre.py --write -o genre_titles.txt

이어지는 채우기 파이프라인:
    python cli.py crawl --platform all --mode search_titles --titles-file titles.txt
    python cli.py batch import        # 또는 scripts/fill_import.py (중복 방지)
"""
import sys
import argparse
...
MISSING_QUERY = """
    ...
    FROM works w
    WHERE w.genre IS NULL OR TRIM(w.genre) = ''
    ORDER BY w.works_id
"""
...
def main():
    ap = argparse.ArgumentParser(description='genre 빈 works 를 titles.txt 로 출력')
    ...
    # 요약 출력
    print(f"\n{'='*70}")
    print(f"genre 빈 works: {len(rows)}건")
    ...
    lines = [
        ...
        '# fill_missing_genre.py 자동 생성 (genre 빈 works)',
